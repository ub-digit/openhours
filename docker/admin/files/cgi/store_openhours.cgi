...
#	print "<h2>Nya &ouml;ppetider som har sparats</h2>\n\r";
	...
	foreach (@loc) {
	    ...
	    foreach (@dagar) {
		@dag_lista = split /\-/, $dagar[$j];
		$insert = "insert into openhours values ($loc_lista[0], \'$dag_lista[0]\', $open, $close, 2, \'$datum\')\;";
		$rv = $dbh->do($insert);
		$j++;
		if ($rv) {
		    $update_ok++;
...
#	print "<p>Antal poster = $tot, Antal insert = $update_ok<br>\n\r";
	if ($tot == $update_ok) {
	    ...
	    $rc = $dbh->disconnect;
	    print "<html><head>\n\r";
	    print "<meta http-equiv=Refresh content=\"1; URL=update_ok.cgi?antal=$tot\">\n\r";
	    print "</head></html>\n\r";
	}
	else {
	    $rc = $dbh->rollback;
